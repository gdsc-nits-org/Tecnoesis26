	...
	>
		<div class="flex flex-1 items-center justify-center">
			{#each images as item, i (item.src)}
				...
				<div
					class={`absolute inset-0 flex origin-[80%_120%] items-center justify-center transition-all duration-[1800ms] ease-out will-change-transform ${
						i === currentIndex
							? 'pointer-events-auto z-20 translate-x-0 translate-y-0 scale-100 rotate-0 opacity-100'
							: i < currentIndex
								? 'pointer-events-none z-10 translate-x-[20%] -translate-y-[120%] scale-[0.85] rotate-[20deg] opacity-0'
								: 'pointer-events-none z-0 translate-x-0 translate-y-[100px] scale-[0.92] rotate-0 opacity-0'
					}`}
				>
