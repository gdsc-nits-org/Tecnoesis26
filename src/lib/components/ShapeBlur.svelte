<script lang="ts">
	...
	let mountRef = $state<HTMLDivElement | null>(null);
...
`;
	...
	// Dynamic uniform updating using Svelte reactive statements
	$effect(() => {
		if (material) {
	...
	onMount(() => {
		...
		const resize = () => {
			if (!active || !mountRef) return;
			w = mountRef.clientWidth;
			h = mountRef.clientHeight;
